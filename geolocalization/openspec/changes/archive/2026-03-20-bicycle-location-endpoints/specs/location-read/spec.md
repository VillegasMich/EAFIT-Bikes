...
The system SHALL expose a GET `/locations` endpoint that returns rows from `bicycles_location` as a JSON array of `LocationResponse` objects. Each object SHALL contain `id`, `bicycle_id`, `latitude`, `longitude`, and `updated_at`. The endpoint SHALL accept an optional query parameter `latest` (boolean, defaults to `false`). When `latest=true`, the endpoint SHALL return only the most recent location per bicycle (by `updated_at`). The response status SHALL be 200.
...
- **WHEN** the client sends GET `/locations/bicycle/:bicycle_id?latest=true` with a valid UUID that has multiple stored locations
...
- **WHEN** the client sends GET `/locations/bicycle/:bicycle_id` with a valid UUID that has no stored locations
- **THEN** the system responds with 200 and an empty JSON array `[]`

#### Scenario: Invalid bicycle ID format
- **WHEN** the client sends GET `/locations/bicycle/:bicycle_id` with a non-UUID string
- **THEN** the system responds with 400 Bad Request

### Requirement: LocationQuery struct
The system SHALL define a `LocationQuery` struct with a single optional field `latest` (bool, defaulting to `false`). The struct SHALL derive `Deserialize`. Both GET handlers SHALL extract this struct from query parameters using Axum's `Query` extractor. The struct SHALL have `///` documentation comments.

#### Scenario: No query parameters provided
- **WHEN** a request has no query parameters
- **THEN** `LocationQuery` deserializes with `latest` set to `false`

#### Scenario: latest=true provided
- **WHEN** a request includes `?latest=true`
- **THEN** `LocationQuery` deserializes with `latest` set to `true`

### Requirement: LocationResponse struct
The system SHALL define a `LocationResponse` struct with the following fields: `id` (UUID), `bicycle_id` (UUID), `latitude` (f64), `longitude` (f64), `updated_at` (DateTime<Utc>). The struct SHALL derive `Serialize` and `Deserialize`. The struct SHALL have `///` documentation comments describing its purpose and fields.

#### Scenario: Serialization matches expected JSON shape
- **WHEN** a `LocationResponse` is serialized to JSON
- **THEN** the output contains keys `id`, `bicycle_id`, `latitude`, `longitude`, `updated_at` with appropriate JSON types (string for UUIDs/datetime, number for coordinates)
