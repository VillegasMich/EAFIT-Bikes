...
### Requirement: bicycle_id column uses INTEGER type
The `bicycles_location` table SHALL define `bicycle_id` as `INTEGER NOT NULL` instead of `UUID NOT NULL`. The database migration SHALL drop the existing table and recreate it with the new schema.

#### Scenario: Table created with integer bicycle_id
- **WHEN** the migration runs on a fresh or existing database
- **THEN** the `bicycles_location` table has a `bicycle_id` column of type `INTEGER`

#### Scenario: Existing data is dropped
- **WHEN** the migration runs on a database with existing UUID-based data
- **THEN** the old table is dropped and recreated with the integer schema
